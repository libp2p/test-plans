...
const simplewebRTC = path.join(scriptDir, '..')
...
(async () => {
    ...
    addGitignoreIfPresent(ig, path.join(simplewebRTC, ".gitignore"))

    const implFolder = path.join(simplewebRTC, 'tests')
    ...
    if (mode == Mode.PushCache) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                console.log(`Pushing cache for ${implFolder}: ${imageID}`)
