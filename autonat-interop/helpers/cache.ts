...
const autonatInteropDir = path.join(scriptDir, '..')
...
(async () => {
    for (const implFamily of fs.readdirSync(path.join(autonatInteropDir, 'impl'))) {
        const ig = ignore()

        addGitignoreIfPresent(ig, path.join(autonatInteropDir, ".gitignore"))
        addGitignoreIfPresent(ig, path.join(autonatInteropDir, "..", ".gitignore"))

        const implFamilyDir = path.join(autonatInteropDir, 'impl', implFamily)
