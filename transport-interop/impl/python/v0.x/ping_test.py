...
# TLS + mplex: peer teardown and mplex background tasks often race; give dialers more time.
    ...
    async def run_listener(self) -> None:
        ...
        try:
            async with self.host.run(listen_addrs=listen_addrs):
                ...
                # Some dialers race the multistream step right after WS upgrade; brief settle time helps.
                ...
                while elapsed < wait_timeout:
                    if self.ping_received:
                        ...
                        # Small delay to allow muxer to drain; in test-plans, wait longer
                        # so the dialer container can exit before we do (see module note).
        ...
        try:
            ...
            async with self.host.run(listen_addrs=dialer_listen_addrs or []):
                ...
                try:
                    ...
                    # Nim / some stacks can race WebSocket + muxer teardown; one retry often succeeds.
                    if (
                        self.test_plans
                        ...
                        print(
                            f"Ping failed ({first_err!r}), retrying once on new stream...",
                            file=sys.stderr,
                        )
                ...
                if self.test_plans:
                    # Three decimal places: compact enough for one-line "Finished:" logs in Node.
                    print(
                        json.dumps(
                            {
