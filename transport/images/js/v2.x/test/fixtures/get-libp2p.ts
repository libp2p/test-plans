...
// Test framework sets uppercase env vars (IS_DIALER, TRANSPORT, SECURE_CHANNEL, MUXER, LISTENER_IP)
...
if (!IP) {
  throw new Error('LISTENER_IP environment variable is required')
}
  ...
  switch (TRANSPORT) {
    ...
    case 'webrtc':
      options.transports = [webRTC(),
        ...
        circuitRelayTransport() // enables circuit relay transport (relay discovery is automatic)
      ]
      options.addresses = {
