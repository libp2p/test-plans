...
source "$SCRIPT_DIR/lib-image-naming.sh"
    ...
    for image_name in "${!unique_images[@]}"; do
        if docker image inspect "$image_name" &> /dev/null; then
            ...
            if [ ! -f "$image_file" ]; then
                ...
                echo "  → [$current_image/$total_images] Saving: $image_name..."
                docker save "$image_name" | gzip > "$image_file"
                local saved_size=$(du -h "$image_file" | cut -f1)
                echo "    ✓ Saved $saved_size"
            else
                echo "  → Skipping $image_name (already saved)"
            fi
        else
            echo "  ⚠ Image not found: $image_name (will need to rebuild on re-run)"
        fi
    done
